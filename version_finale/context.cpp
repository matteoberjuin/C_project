...
void Context::addStaticContactConstraints(float)
{
    ...
    for (int i = 0; i < (int)particles_.size(); ++i)
    {
        ...
        const float d = p.predictedPosition.y - p.radius;
        const float t = p.predictedPosition.y + p.radius;
        const float l = p.predictedPosition.x - p.radius;
        const float r = p.predictedPosition.x + p.radius;
