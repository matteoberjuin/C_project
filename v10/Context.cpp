...
void Context::updatePhysicalSystem(float dt)
{
    ...
    {
        enforceDynamicConstraintSpeed(c,
                                 particles_[c.p1],
                                 particles_[c.p2]);
    }
    ...
    for (size_t i = 0; i < particles_.size(); ++i)
    {
        for (size_t j = i + 1; j < particles_.size(); ++j)
        {
            ...
            if (dist < minDist && dist > 1e-6f)
            {
                ...
                c.normal = d / dist;
                float maxPenetration = 0.01f; // 1 cm (ou moins)
                c.penetration = std::min(minDist - dist, maxPenetration);
                                     ...
                                     Particle& p2)
{
    ...
    // --- 1. Correction de position (anti-pénétration)
...
void Context::enforceDynamicConstraintSpeed(const DynamicConstraint& c,
                                     Particle& p1,
                                     Particle& p2)
{
    float w1 = 1.f / p1.mass;
    ...
    // --- 2. Correction de vitesse (IMPULSION)
    Vec2 vRel = (p2.velocity - p1.velocity)*0.3f;
    float vn = vRel.dot(c.normal);

    // Si les particules s'éloignent déjà, ne rien faire
    if (vn > 0.f)
        return;

    float restitution = 0.5f; // 1 = élastique, <1 = dissipation

    float j = -(1.f + restitution) * vn / wSum;
    Vec2 impulse = j * c.normal;

    p1.velocity -= impulse * w1;
    p2.velocity += impulse * w2;
}
...
void Context::checkPoches()
{
    for (int i = (int)particles_.size() - 1; i >= 0; --i)
    {
        for (const auto& poche : poches_)
        {
            ...
            {
                if (particles_[i].name == "noire") {


                    particles_.erase(particles_.begin() + i);
                    partieTerminee = true;
                    une_bille_est_rentree = false; // Fin du jeu

                }
                else if (particles_[i].name == "boule blanche") {
                ...
                else
                {
                    ...
                    std::string nomBille = particles_[i].name;
                    ...
                    if (nomBille == "rouge")
                    {
                        ...
                        if (joueur_actuel == 1) {
                            une_bille_est_rentree = true;
                        } else {
                            une_bille_est_rentree = false;
                        }
                    }
                    else if (nomBille == "jaune")
                    {
                        score_j2++;
