...
void DrawArea::animate()
{
    ...
    const float rightX = (width() - 2.f * marginPx_) / pixelsPerMeter_;
    const float topY   = (height() - 2.f * marginPx_) / pixelsPerMeter_;
...
void DrawArea::paintEvent(QPaintEvent* /*event*/)
{
    ...
    //tracer barre du haut
    painter.drawLine(
        QPointF(marginPx_, marginPx_),
        QPointF(width() - marginPx_, marginPx_)
        );
    //tracer barre de gauche
    painter.drawLine(
        QPointF(marginPx_, marginPx_),
        QPointF(marginPx_, height() - marginPx_)
        );
    //tracer barre de droite
    painter.drawLine(
        QPointF(width() - marginPx_, marginPx_),
        QPointF(width() - marginPx_, height() - marginPx_)
        );
    ...
    const QPointF a = worldToScreen(Vec2(4.f, 0.f));
    const QPointF b = worldToScreen(Vec2(4.f, 2.5f));
